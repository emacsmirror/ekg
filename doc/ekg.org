...
easiest way, and recommended because it allows you to get the package and
...
* Changelog
** Version 0.9.2
- Fix issue computing batch embeddings without =vecdb=.
** Version 0.9.1
- Improve =ekg-agent= quality and token use
- Allow the user to change the provider for agent commands
- Fix warnings when displaying images
- Improve reliability and fix issues in Apple Notes sync
- Only perform database upgrades when the code changes
** Version 0.9.0
- Added the ability to sync with Apple Notes.
- Changed how note metadata is represented in the note editing UI.
- Added =ekg-agent=, an autonomous agent that can help you with your notes, and perform other tasks using notes as memory.
- Added =ekg-org=, an org-mode task integration module.  Tasks are stored in the ekg database and appear in the org agenda via a virtual file.  Includes =ekg-org-view=, an interactive hierarchical task view for collaborating with AI agents on task management.
- Pushed more configuration into the triple database itself.
** Version 0.8.0
- Add support for the =vecdb= package, for embedding storing and retrieval.
- Add =ekg-truncation-method= variable to control how truncation happens.
- Fix various truncation issues with quotes, underscores and all sorts of other symbols.
** Version 0.7.1
...
- Generate embeddings in batch in =ekg-embedding-generate-all=.
** Version 0.6.3
...
- Add tags while writing the body of notes by completing with "#".  See [[#note-buffer][Note text]] section for details.
- Added functionality so that org-mode commands, and potentially others, narrow automatically so the metadata line and the read-only nature don't mess things up.  See [[#note-buffer][Note text]] section for details.
...
- Made deleting titles and resources possible, and properly skipped empty properties for multi-line properties.
- Handle backup errors better, warning and proceeding if not a forced backup, erroring out with a better message if forced.
...
EKG note buffers use the directory containing the database as their
~default-directory~. Customize ~ekg-notes-default-directory~ to use another
directory instead. Its value can be the symbol ~database~, meaning the directory
containing the database, or a directory name. This normally is only used when
notes want to refer to files by relative filenames, for example for image
display.
...
:PROPERTIES:
:CUSTOM_ID: note-buffer
:END:

The note buffer is a buffer of the particular mode for the node, which can change based off the setting, and can be different for different notes.  It acts mostly just like that mode, but has a header that displays some of the properties of the note, and lets you edit those properties.

The properties shown in the note property area come from the data stored in the
database for the entity. At a minimum, there will be tags. Some properties are
not useful to display (for example, embedding vectors or internal parent/child
IDs). These can be hidden by adding the property keyword to
~ekg-header-hidden-properties~. Packages such as =ekg-embedding= and =ekg-org=
automatically add their internal properties to this list.

Some properties need formatting before display. For example, in the =ekg-org=
module, scheduled dates and deadlines are stored as epoch seconds but should
appear as human-readable dates. The variable ~ekg-property-format-functions~ is an
alist mapping property keywords (e.g. ~:org/scheduled~) to functions that take a
single value and return a display string. Packages can register their own
formatters by adding entries to this alist; =ekg-org= does this automatically for
~:org/scheduled~ and ~:org/deadline~.

For the common cases, there are special commands, available under the =C-c /= prefix keymap:

- =a=: =ekg-note-add-tag=, to add a tag.
- =r=: =ekg-note-remove-tag=, to remove a tag.
- =t=: =ekg-note-add-title=, to add a title.
- =T=: =ekg-note-remove-title=, to remove a title.
- =c=: =ekg-note-change-title=, to change an existing title, or add one if none exist.
...
The buffer has the text of the note.
...
of functionality, but much of it depends on the assumption that the buffer is all
...
true.  In particular, attachments will not work.  However, the =ekg-org= module
(see [[#org-tasks][Org Tasks]]) provides org-agenda integration by presenting ekg tasks as a
virtual org file.
* Capturing notes
...
The adding of templates happens whether initially when setting up the capture
...
- =%n(text <numwords> <format1> <format2> ... <formatn>)=: The text of a note (with any inline commands calculated
  and their results displayed inline).  =numwords= functions as noted above.  The =format= options are an optional set of formatting symbols which control how the text is formatted.  See below for details.
- =%n(tagged <numwords> <format1> <format2> ... <formatn>)=: The tags of a note.
- =%n(time-tracked <format-str> <format1> <format2> ... <formatn>)=: The created and modified time of a note.
  =format-str=, if passed, controls how the times are formatted (see
  documentation for ~format-time-string~, default is ~%Y-%m-%d~).
- =%n(titled <numwords> <format1> <format2> ... <formatn>)=: The title of a note.
...
* Searching for notes
There are a few different ways to search for notes, each with their own advantages and disadvantages.

~ekg-search~ will perform an incrementally displaying list of notes where the text or title matches the query.  It is intended to be used as a quick way to find text.  After the notes have been narrow down to the extent you want, you can press the enter key to make a notes buffer out of the remaining items.  There is no ranking or any semantic sophistication here, it's a straight text match.

~ekg-show-notes-for-query~ will display a notes buffer with entries matching the query, in order of relevance.  This searches for text and titles.  It has some sophistication due to the use of [[https://www.sqlite.org/fts5.html][SQLite's FTS5 extension]].  This should work essentially like a Google query, except it does not accept =AND=, =OR=, or any other search operators.  All terms are treated as required.

In the optional =ekg-embedding= module, there is a third option: =ekg-embedding-search=, which displays a notes buffer similar to =ekg-show-notes-for-query=, but with everything ranked by semantic relevance according to their embeddings.  See the [[#embeddings][embeddings]] section for more details.
* Magic tags
:PROPERTIES:
...
~ekg-add-schema-hook~ is called whenever the database is connected to.  At that
...
#+begin_src emacs-lisp
  (defun my/log-to-ekg (text)
    ...
    (ekg-save-note (ekg-note-create :text text :mode 'text-mode :tags (list (ekg-tag-for-date) "log"))))
#+end_src
...
#+begin_src emacs-lisp
(defun my/log-to-ekg (text)
  "Log TEXT as a note to EKG's date and append if possible."
...
It is highly recommended to install the =vecdb= package (installable on GNU ELPA),
to get access to fast embeddings from a variety of possible backends.  An
example of configuration to do that would be:

#+begin_src emacs-lisp
(use-package vecdb
  :config
  ;; Config for qdrant, other providers for other backends are available, see
  ;; https://github.com/ahyatt/vecdb for details.  For this provider, you will
  ;; need a key and a URL.
  (defvar my-vecdb-provider (make-vecdb-qdrant-provider :api-key <yourkey> :url <url>))
  (setq ekg-vecdb-provider (cons ash/vecdb-provider (make-vecdb-collection :name "ekg" :vector-size 1536))))
#+end_src

If =ekg-vecdb-provider= is not set, the SQLite database is used, which can get
fairly slow when there is a lot of data.
...
first non-date tag as the page where the text will appear, and reference other
...
** Apple Notes
:PROPERTIES:
:CUSTOM_ID: apple-notes
:END:
ekg can sync bidirectionally with Apple Notes on macOS.  This is useful for
accessing and editing your notes on an iPhone or iPad, or for collaborating
with others who use Apple Notes.  The sync uses AppleScript to communicate
with Apple Notes in the background, without bringing the Notes app to the
foreground.

To use this module, you need =pandoc= installed for converting between
org-mode/markdown and HTML (Apple Notes stores content as HTML).
...
*** Setup
Synced notes are stored in a dedicated Apple Notes folder.  By default this
folder is called "ekg", but you can change it with ~ekg-apple-notes-folder~.
The folder is created automatically on the first sync.

Each ekg database can sync to a different Apple Notes folder.  To store the
folder name in the database itself (so it persists and is specific to that
database), use:

#+begin_src emacs-lisp
(ekg-apple-notes-set-folder "my-notes")
#+end_src

If a folder name is stored in the database, it takes precedence over the
~ekg-apple-notes-folder~ customization variable.

If you have multiple Apple Notes accounts (e.g. iCloud and a work account),
you can specify which account to use:

#+begin_src emacs-lisp
(setq ekg-apple-notes-account "iCloud")
#+end_src

To limit which notes are exported, set ~ekg-apple-notes-export-tags~ to a list
of tags.  Only notes with at least one of those tags will be exported:

#+begin_src emacs-lisp
(setq ekg-apple-notes-export-tags '("shared" "mobile"))
#+end_src

Notes with certain tags are excluded from sync entirely.  If =ekg-agent= is
loaded, its tag variables (~ekg-agent-author-tag~ and
~ekg-agent-self-info-tag~) are automatically excluded, so agent notes will
not be synced.  You can exclude additional tags by customizing
~ekg-apple-notes-exclude-tags~.

*** Syncing
The primary command is =ekg-apple-notes-sync=, which performs a bidirectional
sync: first importing from Apple Notes, then exporting to Apple Notes.  You
can also run the two directions separately with =ekg-apple-notes-import= and
=ekg-apple-notes-export=.

The sync tracks timestamps so that only notes modified since the last sync
are processed.  A content hash is stored for each exported note so that
imports can distinguish between notes modified in Apple Notes and notes that
are unchanged from the last export.

*** Tags
ekg tags are stored as individual =Tag: <tag>= lines at the bottom of each
Apple Note.  Each tag gets its own line, preserving the full tag text including
spaces, dots, colons, and other characters.  For example, a note with the tags
"my project" and "date/2024-01-01" would have:
...
These tags don't function as native Apple Notes hashtags, but they are visible
in the note body and can be found via Apple Notes search on any device.

Hidden tags (such as "trash" and "draft") are not synced.

*** Content conversion
Note content is converted between org-mode or markdown and HTML using =pandoc=.
The path to pandoc can be customized with ~ekg-apple-notes-pandoc-executable~.

Links require special handling because Apple Notes strips =<a>= tags when
content is set via AppleScript.  On export, links are converted to markdown
format =[text](url)=, which appears as readable plain text in Apple Notes
(and plain URLs are auto-linked by Apple Notes in its UI).  On import back
to org-mode, these are reconstructed as =[[url][text]]= org links.

*** Limitations
- *Links*: Named hyperlinks degrade to =[text](url)= plain text in Apple Notes.
  Plain URLs are auto-linked by Apple Notes.  On re-import, a heuristic
  reconstructs the links.
- *Formatting*: Apple Notes normalizes HTML aggressively (e.g., =<h1>= becomes
  bold text with a larger font size, =<ol>= becomes =<ul>=).  Repeated
  round-trips may cause minor formatting drift.
- *Attachments*: Images and other attachments in Apple Notes are not synced.
- *Deletion*: Deleting a note in Apple Notes does not automatically delete it
  in ekg (and vice versa).  Deletion sync may be added in the future.
** Email
...
:CUSTOM_ID: org-tasks
:END:
The =ekg-org= module integrates ekg with =org-mode=, allowing ekg notes to serve
as a source of tasks for the org agenda and providing a dedicated task view.
This is not a substitute for =org-mode= --- it is a lightweight, experimental
alternative designed for situations where a human and an AI agent need to
collaborate on a set of tasks, with either party able to create, update, or
reorganize tasks at any time.  The module includes full org-agenda integration,
so tasks managed in ekg appear alongside your regular org tasks.

=ekg-org= is part of the main =ekg= package and does not need to be installed
separately, but it does need to be required:
...
Tasks in =ekg-org= are ordinary ekg notes with special tags and properties:

- *Tags*: Every task has the =org/task= tag.  Its TODO state is stored as a tag
  with the prefix =org/state/= (e.g., =org/state/todo=, =org/state/done=).
  Archived tasks additionally carry the =org/archive= tag.  Any other tags are
  preserved as regular ekg tags.

- *Title*: Each task must have a title (stored in the =:titled/title= property),
  which becomes the org headline when rendered.

- *Hierarchy*: Tasks can have parent-child relationships via the =:org/parent=
  property (an integer note ID).  Children are discovered through a virtual
  reversed property =:org/children=.  There is no limit to nesting depth.

- *Scheduling*: Tasks can have =:org/deadline= and =:org/scheduled= properties
  (stored as unix timestamps), which appear as standard org timestamps.

- *Ordering*: Sibling tasks are ordered by the =:org/sort-order= property
  (defaulting to creation time when unset).

- *Org properties*: Arbitrary org-style properties (uppercase key/value pairs
  following org property syntax) can be attached to tasks via the
  =:org/property= property, and manipulated with =ekg-org-get-property=,
  =ekg-org-set-property=, and =ekg-org-remove-property=.

*** Org agenda integration

When =ekg-org= is loaded, it registers a virtual file =/ekg:tasks.org= with the
org agenda.  This file is generated on the fly from your ekg tasks and is
automatically added to =org-agenda-files=.  No actual file is written to disk;
a file-name handler intercepts Emacs's file operations and returns the generated
content.

This means that =M-x org-agenda= will include your ekg tasks alongside any
other org files you use.  The virtual file is re-generated whenever it is
read, and its modification time tracks the ekg database file, so the agenda
picks up changes automatically.

Archived tasks are served from a separate virtual file =/ekg:tasks.org_archive=.

*** Capturing tasks

You can capture a new org task interactively with =ekg-org-capture=, which
prompts for a title and creates a new note pre-tagged with =org/task= and
=org/state/todo=.  You can also create tasks programmatically:
...
(ekg-save-note
 (ekg-note-create
  ...
  :mode 'org-mode
  :tags '("org/task" "org/state/todo" "my-project")
  :properties '(:titled/title ("Implement feature X"))))
#+end_src

*** Changing task state

The command =ekg-org-change-state= (available in note editing buffers and
=ekg-notes-mode= buffers) prompts for a new TODO state from
=org-todo-keywords-1= and updates the task's state tag accordingly.

*** The ekg-org-view

=ekg-org-view-mode= is a read-only, interactive view of your task hierarchy,
built with the =vui= library.  It presents tasks as an indented outline that
looks like an org buffer (with fontified headings, TODO keywords, and tags) but
is backed entirely by the ekg database.

To open the view, call =ekg-org-view=.  For archived tasks, call
=ekg-org-archive-view=.  To focus on a single task and its subtree, call
=ekg-org-view-task=.

The view refreshes automatically whenever any ekg note is saved, so changes
made by you, by an agent, or by the command-line tools are reflected
immediately.

The keybindings are modeled after org-mode's speed keys (the single-letter
shortcuts available when point is at the beginning of a headline), so they
should feel familiar to org-mode users.

**** Navigation

The following keybindings are available in =ekg-org-view-mode=:

| Key     | Command                          | Description                       |
|---------+----------------------------------+-----------------------------------|
| =n=     | =ekg-org-view-next-heading=      | Move to the next heading          |
| =p=     | =ekg-org-view-previous-heading=  | Move to the previous heading      |
| =f=     | =ekg-org-view-next-sibling=      | Move to the next same-level heading |
| =b=     | =ekg-org-view-previous-sibling=  | Move to the previous same-level heading |
| =u=     | =ekg-org-view-up-heading=        | Move to the parent heading        |

**** Actions

| Key     | Command                          | Description                       |
|---------+----------------------------------+-----------------------------------|
| =t=     | =ekg-org-view-cycle-state=       | Change the TODO state             |
| =TAB=   | =ekg-org-view-toggle-collapse=   | Collapse or expand the subtree    |
| =RET=   | =ekg-org-view-open-note=         | Open the note for editing in ekg  |
| =c=     | =ekg-org-view-create-child=      | Create a child task under point   |
| =C=     | =ekg-org-view-create-sibling=    | Create a sibling task after point |
| =a=     | =ekg-org-view-archive=           | Archive the task at point         |
| =d=     | =ekg-org-view-delete=            | Trash (soft-delete) the task      |
| =L= / =l= | =ekg-org-view-promote=         | Promote: make task a sibling of its parent |
| =R= / =r= | =ekg-org-view-demote=          | Demote: make task a child of its previous sibling |
| =U=     | =ekg-org-view-move-up=           | Move task up (swap with previous sibling) |
| =D=     | =ekg-org-view-move-down=         | Move task down (swap with next sibling) |
| =g=     | =ekg-org-view-refresh=           | Manually refresh the view         |

**** How it differs from org-mode

=ekg-org-view= is intentionally simpler than a full org-mode buffer:

- Tasks are stored in the ekg database, not in files.  This makes them
  accessible to agents, command-line tools, and programmatic APIs without
  needing to parse org syntax.
- The view is read-only.  To edit a task's body text, press =RET= to open it in
  ekg's normal note-editing buffer.
- There is no in-buffer text editing, column view, clocking, or other advanced
  org features.  The focus is on task organization (creating, reordering,
  re-parenting, and state changes).
- Because tasks live in the database, both a human in Emacs and an external
  agent can modify tasks concurrently.  The view auto-refreshes to reflect all
  changes.

*** Agent integration

When =ekg-agent= is loaded alongside =ekg-org=, the agent gains tools for
working with org tasks:

- *add_org_task*: Create a new task with a title, description, tags, optional
  parent, state, deadline, and scheduled date.
- *set_org_task_status*: Change the TODO state of an existing task.
- *list_org_items*: List all active tasks, optionally filtered by state.

These tools are registered automatically by =ekg-org-initialize= when
=ekg-agent= is available.  This allows the agent to create sub-tasks, mark
items done, and review your task list as part of its autonomous operation.

*** Command-line tools

Two command-line scripts are provided in =agent_tools/= for use by external
agents:

- =ekg-org-add-child=: Create a child task under an existing parent.  Supports
  =--title=, =--note=, =--tag=, =--status=, and =--property KEY=VALUE= options.
- =ekg-org-edit=: Edit an existing task's title, content, status, or org
  properties.  Supports =--title=, =--note=, =--append=, =--status=,
  =--set-property KEY=VALUE=, and =--remove-property KEY= options.

These complement the general =ekg-add-note= and =ekg-read= scripts.  See the
=agent_tools/skill.md= file for complete usage documentation.

** Agent
:PROPERTIES:
:CUSTOM_ID: agent
:END:
The =ekg-agent= module provides an autonomous agent that can help you with your
notes.  This agent can review your notes and org-mode agenda, searching across
...
The primary command is =ekg-agent-evaluate-status=, which runs a single session
where the agent reviews your recent activity and decides if it can be of
assistance.  It can create notes to you, or to itself, to help it work.

You can also ask the agent to help you with a specific note you are writing or
editing. The command =ekg-agent-note-response= (bound to =C-c .= by default,
overriding the =ekg-llm= binding) will start an agent loop focused on your
current note. This is an agentic version of =ekg-llm-send-and-append-note=. The
...
called from a note editing buffer, it uses that note.  You can also pass a note
ID programmatically.
...
~ekg-llm-provider~ is a single provider, the prefix argument does not prompt for
a provider.

Internal agent notes (using the =agent/self-info= and =agent/instruct=
tags) are hidden from normal view by being added to =ekg-hidden-tags=. You can
view these notes specifically with =ekg-agent-show-internal-notes=.
...
This will set up a timer to run the agent daily.  You can cancel this schedule
with =ekg-agent-cancel-daily=.

The agent uses specific tags to organize its work:
- ~ekg-agent-author-tag~ (default "agent"): All notes created by the agent will have this tag.
- ~ekg-agent-self-info-tag~ (default "agent/self-info"): The agent uses this tag to store information for itself about your preferences or its own past actions.

You can customize these tags if you prefer different names.

You can also customize the tools available to the agent by modifying
~ekg-agent-extra-tools~. This allows you to add your own tools to the standard set
of tools provided by the agent. We provide two tools that are dangerous but
greatly improve the power of the agent: =ekg-agent-tool-run-elisp=, which enables
the agent to run arbitrary elisp commands, and =ekg-agent-tool-code=, which
enables the agent to integrate with coding tools (Claude Code, Codex, Goose,
etc). To use these tools effectively it's important to run without permissions.
Here's an example:

#+begin_src emacs-lisp
(setq ekg-agent-code-command "~/.local/bin/claude -p --dangerously-skip-permissions")
(setq ekg-agent-extra-tools (list ekg-agent-tool-run-elisp ekg-agent-tool-code))
#+end_src

By default, the coding tool prompt is sent on standard input so prompt contents
do not appear in process listings.  If your command requires the prompt as its
final command-line argument, set =ekg-agent-code-command-prompt-method= to
=argument=.

*** Optional connected-data tool libraries

Some agent tools are packaged as optional libraries because they expose data
from local connected services.  These tools are not enabled by default.  Require
the library you want and add its tool list to =ekg-agent-extra-tools=.

The Gnus tool library provides tools for listing folders, fetching latest
counts, searching messages, reading message headers, and reading full messages
by an opaque =message_id= returned from header and search tools.  Search prompts
are disabled while the tools run so a backend prompt fails instead of blocking
the agent.

#+begin_src emacs-lisp
(require 'ekg-agent-tools-gnus)
(setq ekg-agent-extra-tools
      (append ekg-agent-tools-gnus-tools ekg-agent-extra-tools))
#+end_src

For example, an agent can search for recent messages from a sender whose content
matches a term by calling =gnus_search_messages= with structured arguments such
as =from= set to "Stefan Monnier" and =body= set to "uuid".  It can then pass
the returned =message_id= to =gnus_read_message= to read the message.

The Elfeed tool library provides tools for listing feeds, searching entries
using Elfeed filter syntax, and reading one entry by ID or link.

#+begin_src emacs-lisp
(require 'ekg-agent-tools-elfeed)
(setq ekg-agent-extra-tools
      (append ekg-agent-tools-elfeed-tools ekg-agent-extra-tools))
#+end_src

The standard tool set also includes read-only Emacs reference tools.  The agent
can use =emacs_help_search= and =emacs_help_symbol= to search and display Emacs
help for functions, macros, variables, faces, and other symbols.  It can use
=emacs_info_search= and =emacs_info_node= to search and read the installed Info
manuals, including the Emacs, Emacs Lisp, and Common Lisp extension manuals.
These tools let the agent look up Emacs and Elisp behavior directly instead of
guessing from memory.
*** Agent log mode: continuing and cancelling

When the agent starts a session, it creates a log buffer (named according to
~ekg-agent-log-buffer-name-format~) and enables =ekg-agent-log-mode=, a minor
mode that provides commands for controlling the agent.  The mode-line lighter
shows =Agent:run= while the agent is active and =Agent:idle= when it has stopped.

The following commands are available in the log buffer:

- =C-c C-c= (=ekg-agent-continue=): Continue the agent from where it left off.
  The agent's full conversation history is preserved, so continuing resumes
  exactly where the previous session stopped---whether it completed normally,
  timed out, or was cancelled.  With a prefix argument (=C-u C-c C-c=), you are
  prompted for a message to send to the agent, which lets you give it new
  instructions or redirect its work.

- =C-c C-k= (=ekg-agent-cancel=): Cancel the current agent run.  The agent will
...
For long-running work, the agent is also reminded to call
=summarize_state= if it has gone too long without an explicit progress
update.  You can control this with
~ekg-agent-status-reminder-seconds~.

*** AGENTS.md support

The agent automatically reads =AGENTS.md= files to pick up user-specific
instructions and preferences.  Two locations are checked when the agent's
context is built:

1. =~/AGENTS.md= --- your home directory, for global preferences that apply to
   all projects.
2. =<default-directory>/AGENTS.md= --- the current directory when the agent is
   started, for project-specific instructions.

If either file exists, its contents are appended to the agent's system prompt.
If both exist (and are in different directories), both are included.

In addition, the agent has access to the =read_agents_md= tool, which lets it
read the =AGENTS.md= file from any directory.  This is part of the standard tool
set and requires no configuration.  It allows the agent to discover instructions
for a project directory it encounters during its work, even if that directory
was not the starting directory.

*** Command-line tools for agent integration

The =ekg= package provides command-line tools in the =agent_tools/= subdirectory
that enable external AI agents to use ekg as persistent memory storage.  These
tools communicate with the running Emacs daemon via =emacsclient= and are
designed for programmatic use by agents.

If you use =ekg= with an agent, you need to =(require 'ekg-agent)=.

**** Agent tool location

The command-line tools are located in the =agent_tools/= subdirectory of the
ekg installation:

- =agent_tools/ekg-add-note= - Create notes
- =agent_tools/ekg-read= - Read notes by tag, ID, or search
- =agent_tools/ekg-tags= - Search and list tags
- =agent_tools/ekg-update-tags= - Add or remove tags on existing notes
- =agent_tools/ekg-trash= - Soft-delete notes

To use these tools, agents should locate the ekg installation directory and
reference the scripts with their full path, for example:
=$EKG_DIR/agent_tools/ekg-add-note=

**** Skill file
The file =agent_tools/skill.md= is a complete reference document describing all
available tools, tag conventions, and recommended workflows.  It is designed to
be used as an agent skill --- a file that an agent reads to learn how to use ekg.

To make this skill available to your agent, symlink =skill.md= into wherever
your agent stores its skill files.  For example:

#+begin_src sh
ln -s /path/to/ekg/agent_tools/skill.md ~/.my-agent/skills/ekg/SKILL.md
#+end_src
...
to provide the agent with the =skill.md= file as described above.  For agents
that do not support skill files, you can provide these instructions directly:

***** Long version

#+begin_example
You have access to an emacs-based external memory system called ekg.  It may contain
both notes the user has written, but is also accessible for you to read and write to.
To store information for later retrieval, use the ekg-add-note tool located at:
<ekg-installation-path>/agent_tools/ekg-add-note.  It requires a running emacs
with the daemon running (via the emacs `server-start` command), if there is an issue
connecting, alert the user and ask them to run `server-start`.

Usage:
  ekg-add-note --tag <tag1> --tag <tag2> --note "<note text>" --mode org-mode

Parameters:
  --tag TAG    : Categorize the note (can be used multiple times)
  --note TEXT  : The content to store (required)
  --mode MODE  : Format (org-mode, markdown-mode, or text-mode; default: org-mode)

The system will automatically:
- Mark the note as agent-created (adds "agent" tag)
- Apply automatic tagging functions (e.g., adds current date as "date/YYYY-MM-DD")
- Format the content with appropriate markers
- Store timestamps for creation and modification

Tags may contain spaces or other special characters.  Tags can be hierarchical, some are standard, such as "project/revise event processing/design"

Example usage:
  ekg-add-note --tag "project/revise event processing" --tag "progress" \\
    --note "Completed database optimization. 40% performance improvement."

Use tags strategically:
- Use tag `project/<name>` for project-specific information.
- Use descriptive tags for easy retrieval.
- Multiple tags and nested tags help organize related information.
- Use `agent/instruct` to tag and retrieve information to change the agent behavior.

To retrieve information from your ekg memory, use the ekg-read tool located at:
<ekg-installation-path>/agent_tools/ekg-read

The tool returns JSON-formatted results for easy parsing.

Usage modes:

1. Search by tags (AND logic - notes must have all specified tags):
   ekg-read --tag <tag1> --tag <tag2> -n 10 -w 100

2. Retrieve specific note by ID:
   ekg-read <note-id>

3. Semantic search (finds conceptually similar notes):
   ekg-read --semantic-search "machine learning optimization"

4. Text search (keyword matching):
   ekg-read --search "deadline"

Parameters:
  --tag TAG            : Filter by tag (can be used multiple times for AND logic)
  -n, --num NUM        : Max notes to return (default: 10)
  -w, --words NUM      : Max words per note (default: 100)
  --semantic-search Q  : Find conceptually related notes
  --search QUERY       : Find notes containing keywords
...
[
  {
    ...
    "mode": "org-mode",
    "tags": ["tag1", "tag2"],
    "creation_time": 1768889121,
    "modified_time": 1768889121
  }
]

Example workflow:
  # Retrieve recent project notes
  ekg-read --tag "project/current-work" -n 5

  # Find notes about specific topic
  ekg-read --search "optimization" -n 10

  # Get exact note by ID
  ekg-read 12345678

You should use ekg memory to:
1. Record important findings, decisions, and progress
2. Build up context across multiple interactions
3. Remember user preferences and past discussions
4. Track long-running projects and goals

Workflow:
- At the end of significant interactions, store key information
- Before starting new tasks, retrieve relevant context
- Use tags to organize information hierarchically
- Prefer specific, actionable notes over vague summaries

Example:
  # Store a finding
  ekg-add-note --tag "user-preferences" --tag "coding-style" \\
    --note "User prefers descriptive variable names over short abbreviations"

  # Later, retrieve preferences
  ekg-read --tag "user-preferences" -n 10

Additional tools:

  # Search and list tags
  ekg-tags                              # All tags
  ekg-tags --co-tagged prompt           # Tags co-occurring with "prompt"
  ekg-tags --regex "org/state/.*"       # Tags matching regex
  ekg-tags --prefix "project/"          # Tags with prefix

  # Update tags on a note
  ekg-update-tags <note-id> --add "done" --remove "in-progress"
...
***** Shorter version
#+begin_example
You have access to an emacs-based external memory system called ekg, which stores notes from users and agents with tags.  Use it as a memory when working.  It requires a running emacs
with the daemon running (via the emacs `server-start` command), if there is an issue
connecting, alert the user and ask them to run `server-start`.

To store information for later retrieval, use the ekg-add-note tool located at:
<ekg-installation-path>/agent_tools/ekg-add-note

Usage:
  ekg-add-note --tag <tag1> --tag <tag2> --note "<note text>" --mode org-mode

Tags may contain spaces or other special characters.  Use tags with the "project/" prefix to store information about projects.  Use the `agent/instruct` tag to change future behavior of the agent.
...
To search and list tags, use ekg-tags:
  ekg-tags                              # All tags
  ekg-tags --co-tagged prompt           # Tags co-occurring with "prompt"
  ekg-tags --regex "pattern"            # Tags matching regex

To update tags on a note: ekg-update-tags <note-id> --add tag1 --remove tag2
To soft-delete a note: ekg-trash <note-id>
...
For system administrators configuring agent access:

***** Requirements

- An Emacs daemon must be running for the tools to function
- Start daemon: =emacs --daemon=
- For named daemons: =emacs --daemon=<name>= and use =--daemon <name>= option

***** Implementation

- Tools use =ekg-agent-add-note= and =ekg-agent-read-notes= elisp functions
- Communication via =emacsclient= for fast execution (~0.1 seconds per operation)
- Notes automatically tagged with =ekg-agent-author-tag= (default: "agent")
- Text automatically wrapped in LLM output markers for proper attribution
- JSON encoding via Emacs =json-encode= function

***** Elisp API

For direct elisp integration:

#+begin_src elisp
;; Add a note
(ekg-agent-add-note "Note content" '("tag1" "tag2") 'org-mode)

;; Read notes by tags
(ekg-agent-read-notes :tags '("tag1" "tag2") :num 10 :max-words 100)

;; Read by note ID
(ekg-agent-read-notes :note-id 12345678)

;; Semantic search
(ekg-agent-read-notes :semantic-search "query" :num 10)

;; Text search
(ekg-agent-read-notes :text-search "query" :num 10)

;; List all tags
(ekg-tags)

;; Tags with prefix
(ekg-tags-with-prefix "project/")

;; Tags containing substring
(ekg-tags-including "moltbot")

;; Tags co-occurring with prompt tag
(ekg-agent-cotagged-prompt-tags)
#+end_src
